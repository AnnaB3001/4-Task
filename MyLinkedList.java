...
//        Element minMaxElement = firstElement;
        ...
        if(element != null){
            element.previousPointer = maxElement;
        }
        return true;
    }
    ...
    public void sort() {
        ...
        for (int i = 1; i < quantityElement; i++) {
            if (Double.parseDouble(element.value.toString()) > Double.parseDouble(element.nextPointer.value.toString())) {
                toSwapFirstElement(element);
                element = firstElement;
            }
            for (int j = 1; j < quantityElement - 1; j++) {
                if (Double.parseDouble(element.nextPointer.value.toString()) > Double.parseDouble(element.nextPointer.nextPointer.value.toString())) {
                    toSwap(element);
                }
                element = element.nextPointer;
            }

...
public class JTableUtils {
    ...
    private static void writeListToJTable(JTable table, List<List<String>> list, String itemFormat) {
        ...
        int rank = 1;
        int len1 = list.size(), len2 = -1;
        if (len1 > 0) {
            for (int i = 0; i < len1; i++) {
                List<String> item = list.get(i);
                if (item != null) {
                    rank = 2;
                    len2 = Math.max(item.size(), len2);
                }
            }
        }
        tableModel.setRowCount(rank == 1 ? 1 : len1);
        ...
        for (int i = 0; i < len1; i++) {
            if (rank == 1) {
                tableModel.setValueAt(String.format(itemFormat, Array.get(list.toArray(), i)), 0, i);
            } else {
                Object line = Array.get(list.toArray(), i);
                if (line != null) {
//                    if (line.getClass().isArray()) {
                    int lineLen = list.get(0).size();
                    for (int j = 0; j < lineLen; j++) {
                        tableModel.setValueAt(String.format(itemFormat, list.get(i).get(j)), i, j);
                    }
//                    } else {
//                        tableModel.setValueAt(String.format(itemFormat, Array.get(list.toArray(), i)), 0, i);
//                    }
    ...
    public static void writeListToJTable(JTable table, List<List<String>> array) {
        writeListToJTable(table, array, "%s");
    }

    private static void writeIntListToJTable(JTable table, List<List<Integer>> list, String itemFormat) {
...
//                    }
    ...
    public static void writeIntListToJTable(JTable table, List<List<Integer>> array) {
        writeIntListToJTable(table, array, "%d");
    }

    public static List<List<Integer>> readMatrixFromJTable(JTable table) {
        ...
        for (int i = 0; i < rowCount; i++) {
            for (int j = 0; j < colCount; j++) {
                Integer value = Integer.parseInt(tableModel.getValueAt(i, j).toString());
                lineList.add(value);
            }
    ...
    public static int[][] readIntMatrixFromJTable(JTable table) {
        TableModel tableModel = table.getModel();
        int rowCount = tableModel.getRowCount(), colCount = tableModel.getColumnCount();
        int[][] newArray = new int[rowCount][colCount];
        for (int i = 0; i < rowCount; i++) {
            for (int j = 0; j < colCount; j++) {
                newArray[i][j] = Integer.parseInt(tableModel.getValueAt(i, j).toString());
            }
        }
        return newArray;
    }

    public static int[] readIntArrayFromJTable(JTable table) {
        TableModel tableModel = table.getModel();
        int  colCount = tableModel.getColumnCount();
        int[] newArray = new int[colCount];
        for (int i = 0; i < colCount; i++) {
                newArray[i] = Integer.parseInt(tableModel.getValueAt(0, i).toString());
        }
        return newArray;
    }

    public static void writeMyLinkedListToJTable(JTable table, MyLinkedList array) {

        DefaultTableModel tableModel = (DefaultTableModel) table.getModel();
        int lengthList = array.getQuantityElement();
        tableModel.setColumnCount(lengthList);
        assert array.getFirstElement() == null;

            int counter = 0;
            for(Object value: array){
                tableModel.setValueAt(String.format("%s", value), 0, counter);
                counter++;
            }
        recalcJTableSize(table);
    }
